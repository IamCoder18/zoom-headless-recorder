...
echo "╔═══════════════════════════════════════════════════════╗"
echo "║          ZoomPipePipe CLI Installer                 ║"
echo "╚═══════════════════════════════════════════════════════╝"
...
# Create config
CONFIG_DIR="${HOME}/.zoombie"
mkdir -p "$CONFIG_DIR"
cat > "$CONFIG_DIR/config.json" << EOF
{
  "registry": "${REGISTRY}",
  "recordingsDir": "${HOME}/zoombieordings",
  "apiPort": 8080,
  "vncPort": 6080,
...
WRAPPER="/usr/local/bin/zoombie"

cat > /tmp/zoombie-wrapper << WRAPPER_EOF
#!/bin/bash
CONFIG_DIR="\${HOME}/.zoombie"
...
RECORDINGS_DIR="\${RECORDINGS_DIR:-\${HOME}/zoombieordings}"
...
case "\${1:-}" in
    run)
        ...
        [ -z "\$MEETING_URL" ] && { echo "Usage: zoombie run <url> [password] [duration] [prep] [join] [recordOffset] [leaveOffset] [recordAfter]"; exit 1; }
        ...
        echo "Starting ZoomPipe recorder..."
    ...
    update)
        ...
        ;;
    status)
        docker ps --filter "name=zoombie" --format "{{.Status}}" 2>/dev/null || echo "Not running"
        ;;
    config)
        \$EDITOR "\$CONFIG_FILE" 2>/dev/null || nano "\$CONFIG_FILE" 2>/dev/null || cat "\$CONFIG_FILE"
        ;;
    --help|-h|"")
        echo "ZoomPipePipe CLI - \${IMAGE_NAME}"
        echo ""
        echo "Commands:"
        ...
        echo ""
        echo "Examples:"
        echo "  zoombie run https://zoom.us/j/123456789"
        echo "  zoombie run https://zoom.us/j/123 passcode 3600 60 600 300 -300 1200"
        ;;
esac
WRAPPER_EOF

sudo mv /tmp/zoombie-wrapper "$WRAPPER"
sudo chmod +x "$WRAPPER"

# Create recordings dir
mkdir -p "${HOME}/zoombieordings"
...
echo "Image: $IMAGE_NAME"
echo "CLI: zoombie"
echo "Recordings: ${HOME}/zoombieordings"
echo ""
echo "Quick start:"
echo "  zoombie run https://zoom.us/j/123456789"
echo ""
